...
let to_shadow_t ~name ~passwd sp =
...
let putspent fd sp =
  let name = sp.name |> string_to_char_array in
  let passwd = sp.passwd |> string_to_char_array in
  let spwd = to_shadow_t ~name ~passwd sp in
  putspent' (addr spwd) fd |> ignore
